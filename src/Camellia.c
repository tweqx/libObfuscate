...
static const uint8_t SIGMA[48] = {
    0xa0, 0x9e, 0x66, 0x7f, 0x3b, 0xcc, 0x90, 0x8b, 0xb6, 0x7a, 0xe8, 0x58,
    0x4c, 0xaa, 0x73, 0xb2, 0xc6, 0xef, 0x37, 0x2f, 0xe9, 0x4f, 0x82, 0xbe,
    0x54, 0xff, 0x53, 0xa5, 0xf1, 0xd3, 0x6f, 0x1c, 0x10, 0xe5, 0x27, 0xfa,
    0xde, 0x68, 0x2d, 0x1d, 0xb0, 0x56, 0x88, 0xc2, 0xb3, 0xe6, 0xc1, 0xfd};

static const int KSFT1[26] = {0,  64, 0,   64, 15,  79, 15,  79, 30,
                              94, 45, 109, 45, 124, 60, 124, 77, 13,
                              94, 30, 94,  30, 111, 47, 111, 47};
static const int KIDX1[26] = {0, 0, 4, 4, 0, 0, 4, 4, 4, 4, 0, 0, 4,
                              0, 4, 4, 0, 0, 0, 0, 4, 4, 0, 0, 4, 4};
static const int KSFT2[34] = {
    0,   64, 0,   64, 15,  79, 15, 79, 30, 94, 30, 94, 45, 109, 45, 109, 60,
    124, 60, 124, 60, 124, 77, 13, 77, 13, 94, 30, 94, 30, 111, 47, 111, 47};
static const int KIDX2[34] = {0, 0, 12, 12, 8, 8, 4, 4, 8,  8,  12, 12,
                              0, 0, 4,  4,  0, 0, 8, 8, 12, 12, 0,  0,
                              4, 4, 8,  8,  4, 4, 0, 0, 12, 12};
...
#define SBOX2(n) ((SBOX[(n)] >> 7 ^ SBOX[(n)] << 1) & 0xff)
#define SBOX3(n) ((SBOX[(n)] >> 1 ^ SBOX[(n)] << 7) & 0xff)
#define SBOX4(n) SBOX[((n) << 1 ^ (n) >> 7) & 0xff]

static void ByteWord(const uint8_t *x, uint32_t *y) {
  for (int i = 0; i < 4; i++) {
    y[i] = ((uint32_t)x[(i << 2) + 0] << 24) +
           ((uint32_t)x[(i << 2) + 1] << 16) +
           ((uint32_t)x[(i << 2) + 2] <<  8) +
		   ((uint32_t)x[(i << 2) + 3] <<  0);
  }
...
static void WordByte(const uint32_t *x, uint8_t *y) {
  for (int i = 0; i < 4; i++) {
    y[(i << 2) + 0] = x[i] >> 24 & 0xff;
    y[(i << 2) + 1] = x[i] >> 16 & 0xff;
    y[(i << 2) + 2] = x[i] >>  8 & 0xff;
...
static void RotBlock(const uint32_t *x, const int n, uint32_t *y) {
  int r = n & 31;
...
static void SwapHalf(uint8_t *x) {
  for (int i = 0; i < 8; i++) {
    uint8_t t = x[i];
    x[i] = x[8 + i];
    x[8 + i] = t;
...
static void XorBlock(const uint8_t *x, const uint8_t *y, uint8_t *z) {
  for (int i = 0; i < 16; i++)
    z[i] = x[i] ^ y[i];
}

static void Camellia_Feistel(const uint8_t *x, const uint8_t *k, uint8_t *y) {
  uint8_t t[8];
...
static void Camellia_FLlayer(uint8_t *x, const uint8_t *kl, const uint8_t *kr) {
...
void Camellia_set_key(uint8_t *e, const int n, const uint8_t *k) {
  uint8_t t[64];
  uint32_t u[20];
...
void Camellia_encrypt(const uint8_t *e, const int n, const uint8_t *p,
                      uint8_t *c) {
  XorBlock(p, e + 0, c);
...
void Camellia_decrypt(const uint8_t *e, const int n, const uint8_t *c,
                      uint8_t *p) {
  if (n == 128) {
    XorBlock(c, e + 192, p);
  } else {
