...
#define mul1(b) ((uint8_t)(b))
#define mul2(b) ((uint8_t)((b) >> 7 ? ((b) << 1) ^ 0x1b : ((b) << 1)))
...
static void AddRoundConstant(uint8_t x[ROWS][COLS1024], int columns,
                             uint8_t round, Variant v) {
  switch (v & 1) {
    case 0:
...
/* Subuint8_ts replaces each uint8_t by a value from the S-box */
static void Subuint8_ts(uint8_t x[ROWS][COLS1024], int columns) {
...
/* ShiftBytes cyclically shifts each row to the left by a number of
   positions */
static void ShiftBytes(uint8_t x[ROWS][COLS1024], int columns, Variant v) {
  int *R = Shift[v / 2][v & 1];
  uint8_t temp[COLS1024];
...
/* MixBytes reversibly mixes the uint8_ts within a column */
static void MixBytes(uint8_t x[ROWS][COLS1024], int columns) {
  uint8_t temp[ROWS];
...
static void P(GROSTL512_DATA *grostl, uint8_t x[ROWS][COLS1024]) {
  Variant v = grostl->columns == 8 ? P512 : P1024;

  for (uint8_t i = 0; i < grostl->rounds; i++) {
    AddRoundConstant(x, grostl->columns, i, v);
    Subuint8_ts(x, grostl->columns);
...
static void Q(GROSTL512_DATA *grostl, uint8_t x[ROWS][COLS1024]) {
...
/* digest (up to) msglen uint8_ts */
static void Transform(GROSTL512_DATA *grostl, const uint8_t *input,
                      uint32_t msglen) {
  uint8_t temp1[ROWS][COLS1024], temp2[ROWS][COLS1024];

  /* digest one message block at the time */
  for (; msglen >= (uint32_t)grostl->statesize;
    ...
    for (int i = 0; i < ROWS; i++) {
      for (int j = 0; j < grostl->columns; j++) {
...
static void OutputTransformation(GROSTL512_DATA *grostl) {
  uint8_t temp[ROWS][COLS1024];
...
void Grostl512_init(GROSTL512_DATA *grostl) {
  ...
  for (int i = ROWS - sizeof(int); i < ROWS; i++)
    grostl->chaining[i][grostl->columns - 1] = 512 >> (8 * (7 - i));
...
void Grostl512_data(GROSTL512_DATA *grostl, const uint8_t *buffer, uint32_t len) {
  int index = 0;
  uint32_t msglen = len / 8; /* no. of (full) bytes supplied */
  uint32_t rem = len % 8;    /* no. of additional bits */
  ...
  if (grostl->buf_ptr) {
    /* copy data into buffer until buffer is full, or there is no more
       data */
    for (index = 0;
         grostl->buf_ptr < grostl->statesize && ((uint32_t)index) < msglen;
  ...
  for (; (uint32_t)index < msglen; index++, grostl->buf_ptr++) {
    grostl->buffer[grostl->buf_ptr] = buffer[index];
  }
...
void Grostl512_finalize(GROSTL512_DATA *grostl, uint8_t *hash) {
  int hashuint8_tlen = grostl->hashbitlen / 8;
  ...
  if (grostl->bits_in_last_byte) {
    ...
    grostl->buffer[grostl->buf_ptr++] = 0x80;
  ...
  while (grostl->buf_ptr > grostl->statesize - LENGTHFIELDLEN) {
    grostl->buffer[--grostl->buf_ptr] = (uint8_t)grostl->block_counter;
  ...
  /* store hash output */
  int j = 0;
  for (int i = grostl->statesize - hashuint8_tlen; i < grostl->statesize; i++, j++)
  ...
  for (int i = 0; i < grostl->statesize; i++)
    grostl->buffer[i] = 0;
}
