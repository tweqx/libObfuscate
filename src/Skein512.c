...
#define SKEIN_MK_64(hi32, lo32) ((((uint64_t)hi32) << 32) + (lo32))
...
static uint64_t RotL_64(uint64_t x, uint32_t N) {
  return (x << (N & 63)) | (x >> ((64 - N) & 63));
}

#define BLK_BITS (WCNT * 64)

static uint64_t Skein_Swap64(uint64_t w64) {
  ...
  if (1 == ((uint8_t *)&ONE)[0])
    ...
    return (((w64 >>  0) & 0xFF) << 56) | /*    big-endian is slow */
           (((w64 >>  8) & 0xFF) << 48) |
           (((w64 >> 16) & 0xFF) << 40) |
           (((w64 >> 24) & 0xFF) << 32) |
           (((w64 >> 32) & 0xFF) << 24) |
...
static void Skein_Put64_LSB_First(uint8_t *dst, const uint64_t *src,
                                  uint32_t bCnt) { 
  /* this version is fully portable (big-endian or little-endian), but slow */
  for (uint32_t n = 0; n < bCnt; n++)
    dst[n] = src[n >> 3] >> (8 * (n & 7));
}

static void Skein_Get64_LSB_First(uint64_t *dst, const uint8_t *src,
                                                 uint32_t wCnt) {
  /* this version is fully portable (big-endian or little-endian), but slow */

  for (uint32_t n = 0; n < 8 * wCnt; n += 8)
    dst[n / 8] =
        ((uint64_t)src[n + 0] <<  0) + ((uint64_t)src[n + 1] <<  8) +
        ((uint64_t)src[n + 2] << 16) + ((uint64_t)src[n + 3] << 24) +
        ((uint64_t)src[n + 4] << 32) + ((uint64_t)src[n + 5] << 40) +
        ((uint64_t)src[n + 6] << 48) + ((uint64_t)src[n + 7] << 56);
}

static void Skein_512_Process_Block(SKEIN512_DATA *ctx, const uint8_t *blkPtr,
  ...
  do {
    ...
    for (uint32_t r = 1; r <= SKEIN_512_ROUNDS_TOTAL / 8; r++) {
      /* unroll 8 rounds */
    ...
    blkPtr += SKEIN_512_BLOCK_uint8_tS;
  } while (--blkCnt);
}

void Skein512_init(SKEIN512_DATA *skein) {
  union {
    uint8_t b[SKEIN_512_STATE_uint8_tS];
...
void Skein512_data(SKEIN512_DATA *skein, const uint8_t *buffer, uint32_t len) {
  /* process full blocks, if any */
  if (len + skein->h.bCnt > SKEIN_512_BLOCK_uint8_tS) {
    /* finish up any buffered message data */
    if (skein->h.bCnt) {
      /* # bytes free in buffer b[] */
      uint32_t n = SKEIN_512_BLOCK_uint8_tS - skein->h.bCnt;
      if (n) {
      ...
      Skein_512_Process_Block(skein, skein->b, 1, SKEIN_512_BLOCK_uint8_tS);
      skein->h.bCnt = 0;
    }

    // now process any remaining full blocks, directly from input message data
    if (len > SKEIN_512_BLOCK_uint8_tS) {
      /* number of full blocks to process */
      uint32_t n = (len - 1) / SKEIN_512_BLOCK_uint8_tS;

      Skein_512_Process_Block(skein, buffer, n, SKEIN_512_BLOCK_uint8_tS);

      len -= n * SKEIN_512_BLOCK_uint8_tS;
      buffer += n * SKEIN_512_BLOCK_uint8_tS;
    }
  }
...
void Skein512_finalize(SKEIN512_DATA *skein, uint8_t *hash) {
  uint64_t X[SKEIN_512_STATE_WORDS];

  /* tag as the final block */
  skein->h.T[1] |= SKEIN_T1_FLAG_FINAL;

  /* zero pad b[] if necessary */
  if (skein->h.bCnt < SKEIN_512_BLOCK_uint8_tS)
    memset(&skein->b[skein->h.bCnt], 0,
           SKEIN_512_BLOCK_uint8_tS - skein->h.bCnt);

  /* process the final block */
  Skein_512_Process_Block(skein, skein->b, 1, skein->h.bCnt); 

  /* now output the result */

  /* total number of output bytes */
  uint32_t byteCnt = (skein->h.hashBitLen + 7) >> 3;

  /* run Threefish in "counter mode" to generate more output */
  memset(skein->b, 0,
         sizeof(skein->b));       /* zero out b[], so it can hold the counter */
  memcpy(X, skein->X, sizeof(X)); /* keep a local copy of counter mode "key" */

  for (uint32_t i = 0; i * SKEIN_512_BLOCK_uint8_tS < byteCnt; i++) {
    /* build the counter block */
    ((uint64_t *)skein->b)[0] = Skein_Swap64((uint64_t)i);

    /* run "counter mode" */
    Skein_Start_New_Type(skein, OUT_FINAL);
    Skein_512_Process_Block(skein, skein->b, 1, sizeof(uint64_t)); 

    /* number of output bytes left to go */
    uint32_t n = byteCnt - i * SKEIN_512_BLOCK_uint8_tS; 
    if (n >= SKEIN_512_BLOCK_uint8_tS)
      n = SKEIN_512_BLOCK_uint8_tS;

    /* "output" the ctr mode bytes */
    Skein_Put64_LSB_First(hash + i * SKEIN_512_BLOCK_uint8_tS, skein->X, n); 

    Skein_Show_Final(512, &ctx->h, n, hashVal + i * SKEIN_512_BLOCK_uint8_tS);

    /* restore the counter mode key for next time */
    memcpy(skein->X, X, sizeof(X));
  }
}
