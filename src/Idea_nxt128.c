...
static const uint32_t tbsm0_128[512] = {
...
static const uint32_t tbsm1_128[512] = {
...
static const uint32_t tbsm2_128[512] = {
...
static const uint32_t tbsm3_128[512] = {
...
static const uint32_t tbsm4_128[512] = {
...
static const uint32_t tbsm5_128[512] = {
...
static const uint32_t tbsm6_128[512] = {
...
static const uint32_t tbsm7_128[512] = {
...
static const uint32_t tbs0_128[256] = {
...
static const uint32_t tbs1_128[256] = {
...
static const uint16_t tbs2_128[256] = {
...
#define SIGMA_MU8_0(x, y)                                                      \
  tbsm0_128[(x & 0xff000000) >> 23] ^ tbsm1_128[(x & 0x00ff0000) >> 15] ^      \
  tbsm2_128[(x & 0x0000ff00) >>  7] ^ tbsm3_128[(x & 0x000000ff) <<  1] ^      \
  tbsm4_128[(y & 0xff000000) >> 23] ^ tbsm5_128[(y & 0x00ff0000) >> 15] ^      \
  tbsm6_128[(y & 0x0000ff00) >>  7] ^ tbsm7_128[(y & 0x000000ff) <<  1]
...
#define SIGMA(x)                                                               \
  tbs0_128[(x & 0xff000000) >> 24] ^ tbs1_128[(x & 0x00ff0000) >> 16] ^        \
  tbs2_128[(x & 0x0000ff00) >>  8] ^ tbs3_128[(x & 0x000000ff) >>  0]

#define F64(i)                                                                 \
  {                                                                            \
    uint32_t tmp0 = x0 ^ x1 ^ rk[0];                                           \
    uint32_t tmp1 = x2 ^ x3 ^ rk[1];                                           \
                                                                               \
    uint32_t smu0 = rk[2] ^ SIGMA_MU8_0(tmp0, tmp1);                           \
    uint32_t smu1 = rk[3] ^ SIGMA_MU8_1(tmp0, tmp1);                           \
                                                                               \
    f0 = rk[0] ^ SIGMA(smu0);                                                  \
    f1 = rk[1] ^ SIGMA(smu1);                                                  \
  }
...
void Ideanxt128_encrypt(const nxt128_ctx *ctx, const uint8_t *in,
                        uint8_t *out) {
...
void Ideanxt128_decrypt(const nxt128_ctx *ctx, const uint8_t *in,
                        uint8_t *out) {
  uint32_t x0, x1, x2, x3;
  ...
  const uint32_t *rk = ctx->rk + 4 * (NXT128_TOTAL_ROUNDS - 1);
  uint32_t f0, f1;
...
#endif
#else  /* !NXT128_UNROLL_LOOPS */
  for (int i = 0; i < NXT128_TOTAL_ROUNDS - 1; i++) {
    ELMIO128(0);
  }
  ELMID128(0);
#endif /* !NXT128_UNROLL_LOOPS */
...
static void nxt128_dnl128(const uint8_t *mkey, uint32_t *reg, uint32_t *rkey,
                   uint8_t eq) {
  uint32_t t0[8];
  uint32_t t1[8];
  uint32_t dkey32[8];
  ...
  for (int i = 0; i < 10; i++) {
    LFSR(reg, lfsr_value);
    dkey[0 + i * 3] = mkey[0 + i * 3] ^ (lfsr_value >> 16);
    dkey[1 + i * 3] = mkey[1 + i * 3] ^ (lfsr_value >>  8);
    dkey[2 + i * 3] = mkey[2 + i * 3] ^ (lfsr_value >>  0);
  }

  LFSR(reg, lfsr_value);
  dkey[30] = mkey[30] ^ (lfsr_value >> 16);
  dkey[31] = mkey[31] ^ (lfsr_value >> 8);

  /* NL128-part */
  uint32_t *rk = dkey32;

  PACK32(dkey +  0, dkey32 + 0);
  ...
  uint32_t x0 = SIGMA(t0[0]) ^ SIGMA(t0[4]);
  uint32_t x1 = SIGMA(t0[1]) ^ SIGMA(t0[5]);
  uint32_t x2 = SIGMA(t0[2]) ^ SIGMA(t0[6]);
  uint32_t x3 = SIGMA(t0[3]) ^ SIGMA(t0[7]);
...
void Ideanxt128_set_key(nxt128_ctx *ctx, const uint8_t *key,
                        const uint16_t key_len) {
  const uint16_t ek = 256;
  uint8_t pk[32];
  uint8_t mk[32];
  ...
  uint32_t reg = 0x006a0000 | ((NXT128_TOTAL_ROUNDS & 0xff) << 8) |
        ((~NXT128_TOTAL_ROUNDS) & 0xff);
  if (reg & 0x1)
    reg ^= 0x100001b;
  reg >>= 1;

  uint8_t eq = key_len == ek;

  if (key_len < ek) {
    nxt_p(key, key_len >> 3, pk, ek);
    nxt_m(pk, mk, ek);

    for (int i = 0; i < NXT128_TOTAL_ROUNDS; i++)
