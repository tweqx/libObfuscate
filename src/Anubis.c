 ...
 * @param structpointer	Pointer to the structure that will hold the expanded
 * key.
 */
void Anubis_set_key(ANUBIS_DATA *pAd, const uint8_t *key) {
  ...
  for (int i = 0, pos = 0; i < N; i++, pos += 4) {
    kappa[i] = (key[pos + 0] << 24) ^ (key[pos + 1] << 16) ^
               (key[pos + 2] <<  8) ^ (key[pos + 3] <<  0);
  }
  ...
  for (int r = 1; r < R; r++) {
    for (int i = 0; i < 4; i++) {
      uint32_t v = pAd->roundKeyEnc[R - r][i];
 ...
 */
static void aux_crypt(const uint8_t *plaintext, uint8_t *ciphertext,
  ...
  for (int i = 0, pos = 0; i < 4; i++, pos += 4) {
    state[i] = (plaintext[pos + 0] << 24) ^ (plaintext[pos + 1] << 16) ^
               (plaintext[pos + 2] <<  8) ^ (plaintext[pos + 3] <<  0) ^
               roundKey[0][i];
  }
   ...
   */
  for (int r = 1; r < R; r++) {
    inter[0] = T0[(state[0] >> 24) & 0xff] ^ T1[(state[1] >> 24) & 0xff] ^
               T2[(state[2] >> 24) & 0xff] ^ T3[(state[3] >> 24) & 0xff] ^
               roundKey[r][0];
    ...
    inter[2] = T0[(state[0] >>  8) & 0xff] ^ T1[(state[1] >>  8) & 0xff] ^
               ...
               roundKey[r][2];
    inter[3] = T0[(state[0] >>  0) & 0xff] ^ T1[(state[1] >>  0) & 0xff] ^
               T2[(state[2] >>  0) & 0xff] ^ T3[(state[3] >>  0) & 0xff] ^
               roundKey[r][3];
   ...
   * last round:
   */
  inter[0] = (T0[(state[0] >> 24) & 0xff] & 0xff000000) ^
             (T1[(state[1] >> 24) & 0xff] & 0x00ff0000) ^
             (T2[(state[2] >> 24) & 0xff] & 0x0000ff00) ^
             (T3[(state[3] >> 24) & 0xff] & 0x000000ff) ^ roundKey[R][0];
  inter[1] = (T0[(state[0] >> 16) & 0xff] & 0xff000000) ^
             (T1[(state[1] >> 16) & 0xff] & 0x00ff0000) ^
             (T2[(state[2] >> 16) & 0xff] & 0x0000ff00) ^
             (T3[(state[3] >> 16) & 0xff] & 0x000000ff) ^ roundKey[R][1];
  inter[2] = (T0[(state[0] >>  8) & 0xff] & 0xff000000) ^
             (T1[(state[1] >>  8) & 0xff] & 0x00ff0000) ^
             (T2[(state[2] >>  8) & 0xff] & 0x0000ff00) ^
             (T3[(state[3] >>  8) & 0xff] & 0x000000ff) ^ roundKey[R][2];
  inter[3] = (T0[(state[0] >>  0) & 0xff] & 0xff000000) ^
             (T1[(state[1] >>  0) & 0xff] & 0x00ff0000) ^
             (T2[(state[2] >>  0) & 0xff] & 0x0000ff00) ^
             (T3[(state[3] >>  0) & 0xff] & 0x000000ff) ^ roundKey[R][3];
  ...
  for (int i = 0, pos = 0; i < 4; i++, pos += 4) {
    ...
    ciphertext[pos + 0] = w >> 24;
    ciphertext[pos + 1] = w >> 16;
    ciphertext[pos + 2] = w >>  8;
    ciphertext[pos + 3] = w >>  0;
  }
}
 ...
 */
void Anubis_encrypt(const ANUBIS_DATA *pAd, const uint8_t *plaintext,
                    uint8_t *ciphertext) {
 ...
 */
void Anubis_decrypt(const ANUBIS_DATA *pAd, const uint8_t *ciphertext,
                    uint8_t *plaintext) {
