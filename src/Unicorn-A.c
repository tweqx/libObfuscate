...
static void F(uint32_t ida, uint32_t idb, uint32_t *k, uint32_t *oda,
              uint32_t *odb) {
  uint32_t wx0 = ida + k[0];
  uint32_t wx1 = idb + k[2];
  uint32_t wk0 = idb + k[1];
  uint32_t wk1 = ida + k[3];
  uint32_t tmp = wx0 ^ (wx0 << 23) ^ (wx1 >> 9) ^ (wx0 >> 23) ^ (wx1 < 9);
...
void Unicorn_encrypt(const uint8_t *eKey, const uint8_t *src, uint8_t *dst) {
  uint32_t wx[4];
  for (int i = 0; i < 4; i++) {
    wx[i]  = src[i * 4 + 0] << 24;
    wx[i] |= src[i * 4 + 1] << 16;
  ...
  for (int i = 0; i < 4; i++) {
    wx[i] += *((uint32_t *)(eKey + (IK0 + i * 4)));
  }

  uint32_t tmp[2];
  for (int i = 0; i < ROUND; i++) {
    F(wx[2], wx[3], (uint32_t *)&eKey[EK0 + (i * 16)], &tmp[0], &tmp[1]);
    tmp[0] ^= wx[0];
    tmp[1] ^= wx[1];
    wx[0] = wx[2];
    wx[1] = wx[3];
  ...
  wx[0] -= *((uint32_t *)(eKey + (IK4 +  8)));
  wx[1] -= *((uint32_t *)(eKey + (IK4 + 12)));
  wx[2] -= *((uint32_t *)(eKey + (IK4 +  0)));
  wx[3] -= *((uint32_t *)(eKey + (IK4 +  4)));
...
void Unicorn_decrypt(const uint8_t *eKey, const uint8_t *src, uint8_t *dst) {
  uint32_t wx[4];
  for (int i = 0; i < 4; i++) {
    wx[i]  = src[i * 4 + 0] << 24;
    wx[i] |= src[i * 4 + 1] << 16;
  ...
  wx[0] -= *((uint32_t *)(eKey + (IK0 +  8)));
  wx[1] -= *((uint32_t *)(eKey + (IK0 + 12)));
  wx[2] -= *((uint32_t *)(eKey + (IK0 +  0)));
  wx[3] -= *((uint32_t *)(eKey + (IK0 +  4)));
...
void Unicorn_set_key(uint8_t *eKey, const uint8_t *secret) {
  uint32_t wk[LINE];
  for (int i = 0; i < LINE; i++) {
    wk[i] = secret[i * 4 + 0] << 24 | secret[i * 4 + 1] << 16 |
            secret[i * 4 + 2] <<  8 | secret[i * 4 + 3] <<  0;
  }
  ...
  for (int i = 0; i < 16 * ((ROUND + 2) / 2); i += 16) {
    int j;

    for (j = i; j < (i + 8); j++) {
      wk[(j + 0) % LINE] *= 0x01010101;
