...
static const uint32_t s_box[4][256] = {
...
#define f_rnd(x, n)                                                            \
  f1(x[2], x[3], l_key[n + 0], l_key[n + 4]);                                  \
...
void Cast256_set_key(uint32_t *l_key, const uint32_t *in_key,
...
void Cast256_encrypt(const uint32_t *l_key, const uint32_t *in_blk,
                     uint32_t *out_blk) {
  uint32_t blk[4];
  blk[0] = bswap(in_blk[0]);
  blk[1] = bswap(in_blk[1]);
  blk[2] = bswap(in_blk[2]);
  blk[3] = bswap(in_blk[3]);
  ...
  out_blk[0] = bswap(blk[0]);
  out_blk[1] = bswap(blk[1]);
  out_blk[2] = bswap(blk[2]);
  out_blk[3] = bswap(blk[3]);
}

void Cast256_decrypt(const uint32_t *l_key, const uint32_t *in_blk,
                     uint32_t *out_blk) {
  uint32_t blk[4];
  blk[0] = bswap(in_blk[0]);
  blk[1] = bswap(in_blk[1]);
